...
impl Node {
	...
	Node{name: name, flow: flow, idx: 0 }
    }
}
#[derive(Debug)]
struct Edge {
...
impl Graph {
    fn new() -> Graph {
	Graph { nodes: Vec::new(), edges: Vec::new(), names: HashMap::new() }
    }
	...
	self.edges[n1].push(Edge{src: n1, dest: n2, cost: 1});
    }
...
fn score2(graph: &Graph,
	  dists : &Vec<Vec<i32>>,
	  valves_to_open: &Vec<usize>,
	  path: &Vec<usize>,
	  already_opened: &Vec<usize>,
	  start: usize, current_time: i32, total_time: i32) -> (i32, Vec<usize>)
{
	...
	let next_to_open : Vec<usize> = valves_to_open.iter()
	    .filter(|v| *v != valve)
	    .map(|v| *v)
	    .collect();
	...
	let candidate = score2(graph, dists,
			       &next_to_open,
			       &next_path,
			       &already_opened,
			       *valve,
...
fn func(lines: impl Iterator<Item = String>) {
	...
	let caps = re.captures(line.as_str()).unwrap().iter().map(|c| c.unwrap().as_str()).collect::<Vec<&str>>();
	let id = caps[1];
    ...
    let valves_to_open = (0..num).filter(|n| graph.nodes[*n].flow > 0).collect::<Vec<usize>>();
    dbg!(&valves_to_open);
